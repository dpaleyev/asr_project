...
def collate_fn(dataset_items: List[dict]):
    ...
    result_batch["text_encoded_length"] = Tensor([len(item['text_encoded'][0]) for item in dataset_items]).to(long)
    result_batch["spectrogram"] = pad_sequence([item["spectrogram"][0].T for item in dataset_items], batch_first=True).transpose(1, 2)
    result_batch["spectrogram_length"] = Tensor([item["spectrogram"].shape[2] for item in dataset_items]).to(long)
